// 自由模式：8 个维度 + 拼接逻辑（跟 web 版 app/src/lib/freeMode.ts 同构）

const FREE_DIMENSIONS = [
  {
    id: 'scene',
    name: '场景氛围',
    connector: '立于',
...
const TOTAL_KEYWORDS = FREE_DIMENSIONS.reduce((acc, d) => acc + d.keywords.length, 0);
const TOTAL_DIMENSIONS = FREE_DIMENSIONS.length;

// selection 形如 { scene: ['xxx'], costume: ['yyy'], ... }
function totalSelected(selection) {
  return Object.keys(selection || {}).reduce((acc, k) => {
    const arr = selection[k] || [];
    return acc + arr.length;
  }, 0);
}

function buildFreePrompt(selection) {
  const get = (id) => (selection && selection[id]) || [];
